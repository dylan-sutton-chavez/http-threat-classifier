...
class TrainingInfo:
    def __init__(self, epochs: int, learning_rate: float, dataset_path: str, model_metadata: dict[str, str], patience: float, past_model: dict[str, any] = None):
        """
        initialize an object with the full training info
...
class CachedModel:
    def __init__(self, current_idx: int, weights: list[float], bias: float, means: list[float] = None, standard_deviation: list[float] = None, training_info: TrainingInfo = None):
        """
        ...
        args:
            ...
            training_info: TrainingInfo = None → an object with the training info 
        ...
        self.training_info: TrainingInfo = training_info
...
class ModelCache:
    ...
    def add_item(self, weights: list[float], bias: float, means: list[float], standard_deviation: list[float], training_info: TrainingInfo = None):
        """
        ...
        time comlexity → o(1)
        """
        perceptron = CachedModel(self.current_idx, weights, bias, means, standard_deviation, training_info) # create a new perceptron object
        ...
        with self.lock:
            ...
            self.current_idx += 1 # increment the index in one

        return perceptron.cache_id
    
    def get_item(self, cache_id: int):
        """
        ...
        """
        with self.lock:
            # check if the item id exists win the cache
            if cache_id not in self.cache:
                raise KeyError(f"No perceptron found with id {cache_id}")

            self.cache.move_to_end(cache_id) # order the cache in a LRU format
            return self.cache[cache_id]

    def remove_item(self, cache_id: int):
        """
    ...
    def update_item(self, cache_id: int, weights: list[float] = None, bias: float = None, means: list[float] = None, standard_desviation: list[float] = None, training_info: TrainingInfo = None):
        """
        ...
        args:
            ...
            bias: float → bias of decision for the model
            means: list[float] = None → means of the dataset columns
            standard_desviation: list[float] = None → standar desviation of the dataset columns
            training_info: TrainingInfo = None → object with the full training info
        ...
        with self.lock:
            ...
            # iterate for each attribute un the dictionary
            for attribute in update_data:
                # confirm if its receiving te attribute and update an atribute
                if update_data[attribute] is not None:
                    setattr(perceptron, attribute, update_data[attribute])
...
if __name__ == "__main__":
    ...
    id = perceptron_cache.add_item(weights=[0.6326, 0.5294], bias=0.682, means=[0.5, 0.5], standard_deviation=[0.5773, 0.5773])
    print(f'The `ID` of the created perceptron its: {id}')

    # get a perceptron with a given id
    get_id = perceptron_cache.get_item(id)
    print(f'The object is: {get_id.cache_id}    Current Bias: {get_id.bias}')

    # update the perceptron bias to `1`
    perceptron_cache.update_item(cache_id=id, bias=1)
    ...
    perceptron_cache.remove_item(id)